...
<section>
    <div class="container-fluid" id="ourwork">
        ...
        <div class="row justify-content-md-center"> 
            ...
            <!-- Gallery -->
            <div class="row mx-1 my-1" data-target="#photomodal-{{$image->id}}" data-toggle="modal">
                <img class="work-img rounded" src="{{ Voyager::image($image->image) }}"  data-slide-to="{{ $loop->remaining }}"  alt="no photo" data-target="#myCarousel"> 
            </div>
           
            <!-- Modal -->
            <div class="modal fade" id="photomodal-{{$image->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-body">
                        <div id="myCarousel" class="carousel slide carousel-fade" data-ride="carousel">
                            
                            <ol class="carousel-indicators">
                                <li data-target="#photomodal-{{$image->id}}" data-slide-to="{{ $loop->remaining }}" class="{{ $loop->first ? 'active' : '' }}"></li>
                            </ol>
                            <!-- Carousel -->
                            <div class="carousel-inner">

                                <div class="carousel-item {{ $image->first() ? 'active' : '' }}">
                                    <img src="{{ Voyager::image($image->image) }}" class="modal-img rounded" alt="no photo #{{$image->id}}">
                                </div>
